...
int ch_est_index_global;
float ch_est_array_global[100000000], ch_est_mean_global=0.00;
...
srslte_refsignal_t refsignal_dl;
      ...
      switch (state) {
        ...
        case DECODE_PDSCH:
          ...
          if (decode_pdsch) {
            ...
            nof_trials++;
            ch_est_index_global = nof_trials;
			float ch_est_value, ch_est_mean_value, diff, new_ch_est_value;
			int ff,fz;
			//FILE *fdiff, *festm;
			//fdiff = fopen("/home/prabhat/srsLTE-master/lib/examples/diff_values.txt", "a+");
			//festm = fopen("/home/prabhat/srsLTE-master/lib/examples/estm_values.txt", "a+");
			if(ch_est_index_global<10000) {
				...
				ch_est_value = srslte_chest_dl_get_rsrp_port(&ue_dl.chest, 0);
				
				new_ch_est_value = srslte_refsignal_mbsfn_gen_seq(&refsignal_dl.f1, 0.05f);
				
				ch_est_array_global[ch_est_index_global] = ch_est_value;
				uerate = SRSLTE_VEC_EMA(nof_bits/1000.0f, uerate, 0.001f);
				float elapsed = (float) t[0].tv_usec + t[0].tv_sec*1.0e+6f;    
				fz = fz + 1;
				ff = ch_est_index_global % 100;
				if (ff == 0){ 
					ch_est_mean_global = 0;
					fz = 1;
					}
				ch_est_mean_global = ch_est_mean_global + ch_est_value;
				ch_est_mean_value = ch_est_mean_global/fz;	
				diff = fabs(ch_est_value-ch_est_mean_value);
				PRINT_LINE("\n %d est.value=%f, diff=%f\n", ch_est_index_global,ch_est_value, diff);
				//fprintf(fdiff, "\n %f", diff);
				//fprintf(festm, "\n %f", ch_est_value);
				// PRINT_LINE("\n index=%d, value=%f, mean=%f \n", ch_est_index_global, ch_est_value, ch_est_mean_value);
				if (elapsed != 0.0f) {
